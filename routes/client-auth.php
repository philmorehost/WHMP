...
$router->get('/client/dashboard', function (Request $request, array $params, Container $container): Response {
    ...
    // It would otherwise appear in "Your Active Products & Services" as a
    // $0 "service" — its real price lives on the domains row — and
    // /client/services already excludes it. Keep it out of the dashboard's
    // SERVICES count and active-services list too, so both screens agree.
    // Domains surface through the DOMAINS stat card (/client/domains).
    $carrierProductId = (int) ($db->selectOne("SELECT id FROM products WHERE name = 'Domain Registration' AND status = 'hidden' LIMIT 1")['id'] ?? 0);
    $carrierFilter = $carrierProductId > 0 ? ' AND product_id <> ' . $carrierProductId : '';

    $servicesCount = (int) ($db->selectOne("SELECT COUNT(*) AS c FROM services WHERE client_id = ? AND status = 'active'{$carrierFilter}", [$clientId])['c'] ?? 0);
    ...
    $activeServices = $db->select("SELECT * FROM services WHERE client_id = ? AND status = 'active'{$carrierFilter} ORDER BY created_at DESC LIMIT 5", [$clientId]);
