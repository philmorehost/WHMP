...
- `php bin/queue-worker.php` — processes the async queue (email sending, etc.) when `QUEUE_DRIVER=redis`. Not needed when running on the `sync` fallback (jobs run inline).
