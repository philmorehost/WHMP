...
final class MailCampaignRepository
{
    ...
    public function create(string $subject, string $body, ?int $clientGroupId): int
    {
        $now = (new DateTimeImmutable())->format('Y-m-d H:i:s');

        return (int) $this->db->insert(
            'INSERT INTO mail_campaigns (subject, body, client_group_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
            [$subject, $body, $clientGroupId, 'draft', $now, $now]
        );
    }
