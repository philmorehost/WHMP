...
final class ProductController
{
    ...
    private function extractFields(Request $request): array
    {
        ...
        return [
            ...
            'whm_package_name' => trim((string) $request->input('whm_package_name', '')) ?: null,
            'autosetup' => $autosetup,
            'name' => trim((string) $request->input('name', '')),
            'description' => trim((string) $request->input('description', '')) ?: null,
            'status' => (string) $request->input('status', 'active'),
            'type' => (string) $request->input('type', 'other'),
            'pay_type' => (string) $request->input('pay_type', 'paid') === 'free' ? 'free' : 'paid',
            'is_upsell' => (string) $request->input('is_upsell', '') === '1',
            'free_duration_type' => (string) $request->input('free_duration_type', 'lifetime') === 'days' ? 'days' : 'lifetime',
            'free_duration_days' => $request->input('free_duration_days') !== null && $request->input('free_duration_days') !== '' ? (int) $request->input('free_duration_days') : null,
