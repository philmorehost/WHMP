...
<!-- Add / Update Promotion Card (Moved up) -->
<div class="cv-card" style="margin-bottom:var(--cv-space-4);">
    <h3 style="margin-top:0;">Add / Update Promotion</h3>
    ...
    <p style="color:var(--cv-text-secondary);font-size:var(--cv-text-xs);margin-top:var(--cv-space-2);margin-bottom:0;">
...
<div class="cv-card">
    <form method="post" action="/admin/promotions/delete-multiple" data-confirm="Are you sure you want to delete the selected promotion codes?"><?= csrf_field() ?>
        <div class="cv-datatable__toolbar" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:var(--cv-space-2);">
            <?= $view->partial('partials.table-search', ['target' => '#promotions-table', 'placeholder' => 'Search promotions...']) ?>
            <button type="submit" class="cv-btn cv-btn--danger" id="btn-delete-selected" data-bulk-delete-for=".promo-checkbox" style="display:none;">Delete Selected</button>
        </div>
        <table class="cv-table" id="promotions-table">
            ...
            <?php foreach ($promotions as $promotion): ?>
                <tr>
                    ...
                    <td>
                        <form method="post" action="/admin/promotions/<?= (int) $promotion['id'] ?>/delete" data-confirm="Are you sure you want to delete this promotion?" style="margin:0;"><?= csrf_field() ?>
            ...
            <?php endforeach; ?>
            <?php if ($promotions === []): ?>
                <tr><td colspan="8" style="color:var(--cv-text-secondary);">No promotions yet.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </form>
