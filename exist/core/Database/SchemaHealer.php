...
final class SchemaHealer
{
    ...
    public function heal(): void
    {
        ...
        try {
            // First, auto-run any pending migrations
            $this->migrator->run();
        } catch (Throwable $e) {
            error_log("SchemaHealer: Migration run error: {$e->getMessage()}");
        }

        try {
            // Then verify all critical tables and columns exist
            $this->checkAndCreateMissingTables();
            $this->checkAndCreateMissingColumns();
            $this->checkAndCreateMissingIndexes();
        } catch (Throwable $e) {
            error_log("SchemaHealer: Healing error: {$e->getMessage()}");
        }
    }
    ...
    private function checkAndCreateMissingTables(): void
    {
        try {
            ...
            return; // If INFORMATION_SCHEMA fails, skip
        }
            ...
            foreach ($indexes as $indexName => $columns) {
                try {
                    ...
                    if (($result['count'] ?? 0) === 0) {
                        ...
                        $this->db->statement("ALTER TABLE `{$table}` ADD INDEX `{$indexName}` ({$columnList})");
                    }
                } catch (Throwable $e) {
                    error_log("[SchemaHealer] Index check/create failed for {$table}.{$indexName}: {$e->getMessage()}");
                }
            }
        }
    }
            ...
            foreach ((array) $migration['up'] as $statement) {
                if (is_string($statement)) {
                    try {
                        ...
                        error_log("[SchemaHealer] Failed creating table {$table}: {$e->getMessage()}");
                    }
                } elseif (is_callable($statement)) {
                    try {
                        $statement($this->db);
                    } catch (Throwable $e) {
                        error_log("[SchemaHealer] Failed running migration closure for {$table}: {$e->getMessage()}");
                    }
                }
            }
        } catch (Throwable $e) {
            error_log("[SchemaHealer] Error including migration {$migrationFile}: {$e->getMessage()}");
        }
    }
