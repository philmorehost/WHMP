...
use CodeVault\Database\SchemaHealer;
...
class Kernel
{
    ...
    private function registerCoreBindings(): void
    {
        ...
        $this->container->singleton(SchemaHealer::class, function (Container $c) use ($basePath) {
            return new SchemaHealer(
                $c->make(Database::class),
                $c->make(Migrator::class),
                $basePath
            );
        });
        ...
        $this->container->singleton(CronActivityService::class, function (Container $c) {
            return new CronActivityService($c->make(Database::class));
        });

        $this->container->singleton(CronActivityReportJob::class, function (Container $c) {
            return new CronActivityReportJob(
                $c->make(CronActivityService::class),
                $c->make(EmailDispatcher::class),
                $c->make(SettingsRepository::class),
            );
        });
        ...
        $this->container->singleton(CronScheduler::class, function (Container $c) use ($basePath) {
    ...
    public function handle(Request $request): Response
    {
        ...
        if (is_file($this->basePath('.installed.lock'))) {
            try {
                $this->container->make(SchemaHealer::class)->heal();
            } catch (\Throwable) {
                // Ignore schema healing errors during bootstrap
            }
